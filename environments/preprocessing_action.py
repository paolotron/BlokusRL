...
def preprocess_action(padded_board, pad, n_p, n_v,
    ...
    #   - turn an invalid action of the active player from invalid to potentially valid (creation of new attachment points)
    #   - turn an action of each player to invalid (overlapping)
    #   - turn an action of the current player to invalid (edge of same color contacting)
    # also computes action always invalid and action valid at start
    # TODO: reduce action space using a dicitonary for conversion
    
    # output file
    actions_file = Path('environments/preprocessed_actions.npz')
    ...
    if not os.path.isfile(actions_file):
        ...
        # saves action data using pickle
        action_data_tuple = {
            'always_invalid': always_invalid,
            'invalid_to_maybe_valid': invalid_to_maybe_valid,
            'valid_to_invalid': valid_to_invalid,
            'valid_to_invalid_act_pl': valid_to_invalid_act_pl,
            'valid_at_start': valid_at_start
            }
        np.savez(actions_file, **action_data_tuple)

    else:
        # loads action data
        action_data_tuple = np.load(actions_file)
    action_data_tuple = tuple(action_data_tuple.values())
    
    # returns actions data
    return action_data_tuple
